...
# 测试环境Web域名
web_host = 'http://192.168.10.194:5555'
web_login_headers = {"sign": "1", "Content-Type": "application/json"}
web_session = requests.session()

# 本地测试地址
app_host = 'http://192.168.10.194:5555'
...
def replace_relate_param(case_name):
    ...
    return str(case_param).encode('utf-8')

# json.loads(str(case_param).encode('utf-8'))  str(case_param).encode('utf-8')
...
def exec_web_request(case_name):
    ...
    # request_content = str(request_content).encode('utf-8')
    ...
    try:
        ...
        assert_that(request_result.json()['code']).is_not_equal_to(100064)
    except Exception as exception_info:
        # 写入result文件
        with open(result, mode='a+', encoding='utf-8') as str_transfer_to_list:
            str_transfer_to_list.write(str("用例名称: " + str(case_name) + "\n"))
            # str_transfer_to_list.write(str("code: " + str(request_result.json()['code']) + "\n"))
